...
data "aws_iam_policy_document" "ecs_task_access_secrets" {
  statement {
    ...
    resources = [
      "arn:${var.aws_ssm_path}:ssm:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:parameter${var.webhook_ssm_parameter_name}",
      "arn:${var.aws_ssm_path}:ssm:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:parameter${var.atlantis_github_user_token_ssm_parameter_name}",
      "arn:${var.aws_ssm_path}:ssm:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:parameter${var.atlantis_gitlab_user_token_ssm_parameter_name}",
      "arn:${var.aws_ssm_path}:ssm:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:parameter${var.atlantis_bitbucket_user_token_ssm_parameter_name}",
    ]
